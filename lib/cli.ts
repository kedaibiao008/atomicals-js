...
program
  ...
  .action(async (addressOrAlias, options) => {
    ...
    const result: { output: any; address: string } =
...
program
  .command("split")
  .description(
    "Split operation to seperate the FT Atomicals at a single UTXOs."
  )
  .argument("<locationId>", "string")
  .option(
    "--funding <string>",
...
program
  .command("disable-subrealm-mints")
...
program
  ...
  .option(
    "--meta <string...>",
    "Populate the 'meta' field with key value pairs or file contents"
  )
  .option(
    "--ctx <string...>",
    "Populate the 'ctx' field with key value pairs or file contents"
  )
  .option(
    "--init <string...>",
    "Populate the 'init' field with key value pairs or file contents"
  )
  .option(
    "--initialowner <string>",
...
program
  ...
  .option(
    "--mintbitworkc <string>",
    "Whether to require any bitwork proof of work to mint. Applies to the commit transaction."
  )
  .option(
    "--mintbitworkr <string>",
  ...
  .action(async (ticker, mintAmount, maxMints, mintHeight, files, options) => {
    try {
      ...
      const mintBitworkc = options.mintbitworkc
        ? options.mintbitworkc
        : getRandomBitwork4();
...
program
  ...
  .action(async (files, options) => {
    const startString = process.env.START || "1000";
    const endString = process.env.END || "9999";
    const stepString = process.env.STEP || "1";
    for (
      let i = parseInt(startString, 10);
      i <= parseInt(endString, 10);
      i = i + parseInt(stepString, 10)
    ) {
      try {
        const requestUrl = `https://server.atomicalmarket.com/mainnet/v1/atommap/block/${i}`;
        const response = await axios.get(requestUrl);
        console.log(response.data);
        if (response.data.code == 404) {
          console.log(`mint ${i}`);
        } else {
          console.log("have mint");
          continue;
        }
        const formattedString = `/root/Atom-map/${i}.atommap.svg`;
