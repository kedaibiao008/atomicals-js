...
async function readMintedFromCSV(workdc: string): Promise<boolean> {
  try {
    console.log("workdc:" + workdc);
    const w = workdc.replace("ab", "");
    const requestUrl = `https://server.atomicalmarket.com/mainnet/v1/atommap/block/${w}`;
    ...
    console.error("Error reading from mint.csv:", error);
    return false; // 或者根据你的需求返回其他默认值
  }
}
...
export class AtomicalOperationBuilder {
  ...
  setData(data: any, log = false) {
    ...
    if (data["args"]) {
      throw new Error(
        `Data cannot have field named 'args' set manually. Use setArgs method.`
      );
    }
    if (data["meta"]) {
      throw new Error(
  ...
  async start(fundingWIF: string): Promise<any> {
    ...
    if (performBitworkForCommitTx) {
      ...
      do {
        ...
        noncesGenerated++;
        if (noncesGenerated % 50000 == 0) {
          if (!(await readMintedFromCSV(copiedData["args"].bitworkc))) {
            performBitworkForCommitTx = false;
          }
        }
      } while (performBitworkForCommitTx);
    } else {
      const baseCommit: { scriptP2TR; hashLockP2TR } =
        prepareCommitRevealConfig(
          this.options.opType,
          fundingKeypair,
    ...
    do {
      ...
      noncesGenerated++;
      if (noncesGenerated % 50000 == 0) {
        if (!readMintedFromCSV(copiedData["args"].bitworkc)) {
